...
def probe_gemm_with_sparse_logic(api, c_init, num, add_val):
    ...
    # b_torch = torch.zeros((K, N), dtype=torch.float16, device=device)
        # 先构造 FP8 矩阵
    a_torch_fp8 = torch.full((M, K), 0.0,dtype=torch.float4_e2m1fn_x2, device=device)
    b_torch_fp8 = torch.full((K, N), 0.0,dtype=torch.float8_e4m3fn, device=device)
    
    # 填充数值
    a_torch_fp8[0, :int(num)] = 1.0
    b_torch_fp8[:int(num), 0] = 1.0
    
    # 转换为 FP4 (这里使用torch的自动类型转换)
    a_torch = a_torch_fp8.to(torch.float4_e2m1fn_x2)
    b_torch = b_torch_fp8.to(torch.float4_e2m1fn_x2)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--api', type=str, default='mm', choices=['mm', 'matmul', 'at', 'addmm'])
    parser.add_argument('--c', type=float, default=(1<<26), help='初始值, 如 2^24')
